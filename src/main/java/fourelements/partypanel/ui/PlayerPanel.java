...
import fourelements.partypanel.PartyPanelConfig;
...
import fourelements.partypanel.ui.equipment.EquipmentPanelSlot;
import fourelements.partypanel.ui.equipment.PlayerEquipmentPanel;
import fourelements.partypanel.ui.prayer.PlayerPrayerPanel;
import fourelements.partypanel.ui.prayer.PrayerSlot;
import fourelements.partypanel.ui.skills.PlayerSkillsPanel;
...
public class PlayerPanel extends JPanel
{
	...
	private static final int ICON_RESIZE_WIDTH = 24;
	private static final int ICON_RESIZE_HEIGHT = 24;
	...
	{
		return new ImageIcon(ImageUtil.resizeImage(image, ICON_RESIZE_WIDTH, ICON_RESIZE_HEIGHT));
	}
		...
		SwingUtilities.invokeLater(() ->
		{
			...
			if (player.getPrayers() != null && tabMap.getOrDefault(SpriteID.TAB_PRAYER, false))
			{
				...
				for (final Map.Entry<Prayer, PrayerSlot> entry : prayersPanel.getSlotMap().entrySet())
				{
					...
					if (data != null)
					{
						unlockChanged |= data.isUnlocked() != slot.getData().isUnlocked();
	...
	public void updatePanel()
	{
		...
		this.setBorder(new CompoundBorder(
				new MatteBorder(2, 2, 2, 2, BORDER_COLOR),
				showInfo ? new EmptyBorder(0, 0, 5, 0) : new EmptyBorder(0, 0, 0, 0)
		));
